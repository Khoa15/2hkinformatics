...
if(isset($_POST['nproduct']) && isset($_POST['price']) && !empty($_POST['nproduct']) && isset($_POST['id'])){
    ...
    if($bphone){
        ...
        if($db->update('products', array(
            ...
            $s = $db->db_get_row('select store_id from products where id='.$pid);
                        ...
                        if(!is_array($value2)){
                            ...
                            if(sizeof($value2)!=0){
                                foreach($value2 as $key => $product){
                                    ...
                                    if($namegroup){
                                    $db->insert('types_of_products', array('name'=>$namegroup, 'type'=>$nprice[0], 'price'=>$price, 'product_id'=>$pid, 'store_id'=>$s['store_id']));
                                    }
                                }
                            }
                        }
