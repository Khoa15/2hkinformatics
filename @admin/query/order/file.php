...
if($id){
	...
	if(!empty($cart['id'])){
		...
		if(!empty($system['ship']) && !empty($product['name']) && !empty($address['fullname'])){
			...
			$amount = $cart['amount'];
			$ship = number_format($system['ship'],0,',',',').'đ x '.$cart['amount'];
			$total2 = ($cart['price'] - $cart['price']*$discount/100);
			$total1 = $total2 * $amount;
			$total1 = $total1 + $system['ship'] * $amount;
			$total = number_format($total1,0,',',',').'đ';
			$coupon2 = null;
			if(!empty($coupon['code'])){
				$code = $coupon['code'];
				$coupon2 = 'Có mã giảm giá được giảm '.$discount.'% còn: '.number_format($total2,0,',',',').' x '.$cart['amount'];
			}
			...
			$xml = str_replace('{id}', $cart['product_id'], $xml);
			$xml = str_replace('{price}', $price, $xml);
			//$xml = str_replace('{coupon}', $price, $xml);
