...
if(isset($_POST['id']) && isset($_POST['sts']))
{
	...
	if($bphone){
		$pstore = $db->db_get_row('select store_id from products where id='.$id);
		$arr['msg'] = 'Không tìm thấy sản phẩm';
		if(!empty($pstore)){
			$store = $db->db_get_row('select actived from stores where id='.$pstore['store_id']);
			$arr['msg'] = 'Cửa hàng của sản phẩm này đang trong trạng thái dừng bán';
			if($store['actived']){
				$arr =array('sts'=>0,'msg'=>'Có lỗi trong quá trình thực hiện');
